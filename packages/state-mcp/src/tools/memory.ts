...
export function registerMemoryTools(server: McpServer, db: CampaignDB) {
  ...
  server.registerTool(
    "recall_memory",
    {
      description: "Search campaign memory using full-text search. Returns relevant chunks from session summaries, NPC notes, lore. Use keyword-rich queries for best results.",
      inputSchema: {
  ...
  server.registerTool(
    "add_memory",
    {
      description: "Manually add a memory chunk to the campaign (for DM injections, important moments, etc).",
      inputSchema: {
